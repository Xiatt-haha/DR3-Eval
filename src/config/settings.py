...
# API for iflow Web Scraping (alternative to Jina, used for ablation experiments)
...
def create_mcp_server_parameters(cfg: DictConfig, agent_cfg: DictConfig):
    ...
    # Google Search with iflow API for web scraping (alternative to Jina)
    # Used for ablation experiments comparing offline RAG vs online search
    if (
        agent_cfg.get("tools", None) is not None
        and "tool-google-search-iflow" in agent_cfg["tools"]
    ):
        if not SERPER_API_KEY:
            raise ValueError(
                "SERPER_API_KEY not set, tool-google-search-iflow will be unavailable."
            )
        if not IFLOW_API_KEY:
            raise ValueError(
                "IFLOW_API_KEY not set, tool-google-search-iflow will be unavailable."
            )

        configs.append(
            {
                "name": "tool-google-search-iflow",
                "params": StdioServerParameters(
                    command=sys.executable,
                    args=[
                        "-m",
                        "miroflow_tools.mcp_servers.searching_google_iflow_mcp_server",
                    ],
                    env={
                        "SERPER_API_KEY": SERPER_API_KEY,
                        "SERPER_BASE_URL": SERPER_BASE_URL,
                        "IFLOW_API_KEY": IFLOW_API_KEY,
                        "IFLOW_API_URL": IFLOW_API_URL,
                    },
                ),
            }
        )
    ...
    if (
        agent_cfg.get("tools", None) is not None
        and "tool-rag" in agent_cfg["tools"]
    ):
        configs.append(
            {
                "name": "tool-rag",
                "params": StdioServerParameters(
                    command=sys.executable,
                    args=["-m", "miroflow_tools.mcp_servers.rag_mcp_server"],
                    env={
                        "OPENAI_API_KEY": OPENAI_API_KEY,
                        "OPENAI_BASE_URL": OPENAI_BASE_URL,
                    },
...
def expose_sub_agents_as_tools(sub_agents_cfg: DictConfig):
    ...
    for sub_agent in sub_agents_cfg.keys():
        # Skip sub_agents that are set to null (disabled)
        if sub_agents_cfg[sub_agent] is None:
            continue
        if "agent-rag-search" in sub_agent:
...
def get_env_info(cfg: DictConfig) -> dict:
    ...
    return {
        ...
        **{
            ...
            if cfg.agent.sub_agents[sub_agent] is not None
        },
