...
# Import RAG log path setter and allowed db path setter
try:
    from miroflow_tools.mcp_servers.rag_mcp_server import set_retrieval_log_path, set_allowed_db_path, clear_allowed_db_path
except ImportError:
    set_retrieval_log_path = None
    set_allowed_db_path = None
    clear_allowed_db_path = None
...
async def run_folder_task(
    ...
    # Set up allowed db path whitelist for RAG tools
    if set_allowed_db_path is not None and target_db_path is not None:
        set_allowed_db_path(target_db_path)
        logger.info(f"RAG allowed db path set to: {target_db_path}")
...
async def run_folder_task_simple(
    ...
    # Run task
    return await run_folder_task(cfg, folder_path, query, target_db_path=target_db_path)
