...
from ..llm.exceptions import APIConnectionSkipError, APIRateLimitError, APITimeoutError
...
class Orchestrator:
    ...
    async def _handle_llm_call(
        ...
        while retry_count < max_retries:
            try:
                ...
                # These exceptions should be propagated to the caller for proper handling
                # (e.g., skipping the task and moving to the next one)
                self.task_log.log_step(
                    "error",
                    f"{purpose} | LLM Call ERROR - SKIPPING TASK",
                    f"{purpose} failed: {str(e)} | Will skip this task",
                )
                # Re-raise the exception so it can be caught by pipeline.py
                raise
    ...
    async def run_sub_agent(
        ...
        Note: APIConnectionSkipError, APIRateLimitError, and APITimeoutError are propagated
        to the caller for proper handling (e.g., skipping the task).
        """
        self.task_log.log_step(
        ...
        if use_cn_prompt == "1":
            task_description += "\n\nPlease provide the answer for this subtask with detailed evidence or supporting information."
        else:
    ...
    async def run_main_agent(
        ...
        while turn_count < max_turns:
            ...
            for call in tool_calls:
                ...
                except (APIConnectionSkipError, APIRateLimitError, APITimeoutError):
                    # These exceptions should be propagated to the caller for proper handling
                    # (e.g., skipping the task and moving to the next one)
                    raise
        ...
        if enable_report_validation and final_answer_text and final_answer_text != "No final answer generated.":
            ...
            while validation_turn < max_validation_turns:
                ...
                if "✅" in final_validation_response or "Validation Passed" in final_validation_response:
                    self.task_log.log_step(
                        "info",
                        f"Main Agent | Validation Turn: {validation_turn}",
                ...
                elif "❌" in final_validation_response or "Needs Revision" in final_validation_response:
                    self.task_log.log_step(
                        "info",
                        f"Main Agent | Validation Turn: {validation_turn}",
