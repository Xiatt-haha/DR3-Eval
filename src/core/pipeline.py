...
async def execute_task_pipeline(
    ...
    try:
        ...
        task_log.log_step(
            "error",
            "🧠 FATAL ERROR | main | LLM Call ERROR",
            f"main failed: {str(e)} | Original error: {getattr(e, 'original_error', 'N/A')}",
        )
        task_log.status = "skipped"
        task_log.error = error_details
        task_log.save()
        
        # Re-raise the exception so it can be caught by main.py batch
        raise
...
def create_pipeline_components(cfg: DictConfig):
    ...
    for sub_agent in cfg.agent.sub_agents:
        ...
        if cfg.agent.sub_agents[sub_agent] is None:
            continue
