...
class BaseClient(ABC):
    ...
    async def create_message(
        ...
        Note: APIConnectionSkipError, APIRateLimitError, and APITimeoutError are NOT caught here.
        They are propagated to the caller for proper handling (e.g., skipping the task).
        """
        from .exceptions import APIConnectionSkipError, APIRateLimitError, APITimeoutError
        ...
        try:
            ...
            self.task_log.log_step(
                "error",
                f"FATAL ERROR | {agent_type} | LLM Call ERROR - SKIPPING TASK",
                f"{agent_type} failed: {str(e)} | TraceID: {e.get_trace_id() if hasattr(e, 'get_trace_id') else 'N/A'}",
            )
            # Re-raise the exception so it can be caught by the orchestrator/pipeline
            raise
